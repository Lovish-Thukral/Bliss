...
const fetchApi = async (data) => {
  try {
    const res = await axios.post(
      'https://bliss-7r87.onrender.com/api/user/signup',
      data
    );
    ...
    <SafeAreaView className="flex-1 bg-pink-50 items-center pt-10">
      ...
      <View>
        ...
        <Text className="text-sm text-gray-500 mt-2">
          Make it tough — so only you can unlock your world.
        </Text>
        ...
        {/* Inline warning if criteria not met */}
        {password.length > 0 && !passwordMeetsCriteria(password) && (
          <Text className="text-red-400 text-xs mt-1 ml-2">
            Password must include letters, numbers, and a symbol.
          </Text>
        )}
        ...
        <TouchableOpacity
          ...
          className={`mt-6 py-3 rounded-full items-center ${
            isValid ? 'bg-pink-500' : 'bg-pink-300'
          }`}
        >
          {loading ? (
            <ActivityIndicator color="white" />
          ) : (
            <Text className="text-white font-semibold text-base w-[100%] text-center">
              Continue
            </Text>
          )}
        </TouchableOpacity>
